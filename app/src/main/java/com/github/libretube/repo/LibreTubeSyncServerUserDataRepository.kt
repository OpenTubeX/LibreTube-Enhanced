...
class LibreTubeSyncServerUserDataRepository : UserDataRepository {
    ...
    override suspend fun getWatchHistory(
        ...
        return tryHttpOrRaiseError {
            ...
            val videos = api.getWatchHistory(page, pageSize, state?.name?.lowercase())

            val hasNext = videos.size == pageSize
            val nextPageCursor = if (hasNext) page + 1 else null

            videos.map { it.toWatchHistoryEntry() } to nextPageCursor
        }
    }
